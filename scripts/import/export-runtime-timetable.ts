  ...
  try {
    ...
    for (const run of runs) {
      ...
      for (let fromIndex = 0; fromIndex < stops.length - 1; fromIndex += 1) for (let toIndex = fromIndex + 1; toIndex < stops.length; toIndex += 1) {
        ...
        services.push({ id: `${run.id}:${from.nodeId}-${to.nodeId}`, mode: run.mode, fromNodeId: from.nodeId, toNodeId: to.nodeId, departureAt: from.departureAt, arrivalAt: to.arrivalAt, cost: 0, fareDataAvailable: false, routeId: run.routeId, operator: run.operator, serviceName: run.serviceNumber });
      }
    }
...
async function main(): Promise<void> {
  ...
  const manifest = { ...(runtime.metadata as Record<string, unknown>), counts: { nodes: (runtime.nodes as unknown[]).length, services: (runtime.services as unknown[]).length, transferRules: (runtime.transferRules as unknown[]).length }, runtimeSha256: createHash("sha256").update(body).digest("hex"), factualFieldsOnly: true, actualOperationsClaimed: false };
  await mkdir(dirname(outputPath), { recursive: true });
  await writeFile(outputPath, body, "utf8");
  await writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
  console.log(JSON.stringify({ outputPath, manifestPath, counts: manifest.counts }));
}

...
describe("Taiwan journey WebMCP tools", () => {
  ...
  it("discovers the earliest goal-completing 0636 train after the delay", async () => {
    const replan = registerTools().get("replan_taiwan_journey")!;
    const output = await json(replan);
    expect(JSON.stringify(output.plan)).not.toContain("THSR_1634_20260831:1060-1020");
    expect(JSON.stringify(output.plan)).toContain("THSR_0636_20260831:1060-1020");
    expect(output).toMatchObject({ safetyMarginMinutes: 195, recommendedJourney: { legs: [{ serviceId: "THSR_0636_20260831:1060-1020" }] } });
  });

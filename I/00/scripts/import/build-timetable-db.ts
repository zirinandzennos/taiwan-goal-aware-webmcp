...
export function buildTimetableDatabase(snapshot: NormalizedTimetableSnapshot, outputPath: string): void {
  ...
  try {
    ...
    database.prepare("INSERT INTO snapshots VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(m.snapshotId, m.source, m.provider, m.dataset, m.dataType, m.retrievedAt, m.periodStart, m.periodEnd, m.timezone, m.license, m.attribution, m.normalizationVersion);

...
export function runOfficialGoldenJourney(): Record<string, unknown> {
  ...
  const goal: JourneyGoal = {
    ...
    completion: {
      access: {
        mode: "WALK",
        fromNodeId: goalData.destinationNodeId,
        toNodeId: "xpark-entrance",
        destinationLabel: "Xpark entrance",
        durationMinutes: goalData.goalActionBufferMinutes,
        source: {
          label: "Xpark official THSR access guidance",
          url: goalData.source.accessUrl,
          retrievedAt: goalData.source.retrievedAt,
        },
      },
    },
